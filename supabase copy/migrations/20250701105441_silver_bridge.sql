/*
  # Migration complète pour SecureCode
...
-- Ajouter les contraintes CHECK pour vulnerabilities si elles n'existent pas
DO $$
BEGIN
  -- Contrainte pour type
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'vulnerabilities_type_check' 
    AND table_name = 'vulnerabilities'
  ) THEN
    ALTER TABLE vulnerabilities ADD CONSTRAINT vulnerabilities_type_check 
    CHECK (type IN ('xss', 'injection', 'secrets'));
  END IF;
  
  -- Contrainte pour severite
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'vulnerabilities_severite_check' 
    AND table_name = 'vulnerabilities'
  ) THEN
    ALTER TABLE vulnerabilities ADD CONSTRAINT vulnerabilities_severite_check 
    CHECK (severite IN ('critique', 'eleve', 'moyen', 'faible'));
  END IF;
  
  -- Contrainte pour confidence
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'vulnerabilities_confidence_check' 
    AND table_name = 'vulnerabilities'
  ) THEN
    ALTER TABLE vulnerabilities ADD CONSTRAINT vulnerabilities_confidence_check 
...
-- Ajouter la contrainte unique pour learning_progress si elle n'existe pas
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Ajouter la contrainte unique pour achievements si elle n'existe pas
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
...
-- Ajouter les contraintes pour user_api_configs si elles n'existent pas
DO $$
BEGIN
  -- Contrainte unique pour user_id, provider
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_api_configs_user_provider_key' 
    AND table_name = 'user_api_configs'
  ) THEN
    ALTER TABLE user_api_configs ADD CONSTRAINT user_api_configs_user_provider_key UNIQUE(user_id, provider);
  END IF;
  
  -- Contrainte pour provider
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_api_configs_provider_check' 
    AND table_name = 'user_api_configs'
  ) THEN
    ALTER TABLE user_api_configs ADD CONSTRAINT user_api_configs_provider_check 
    CHECK (provider IN ('openai', 'anthropic', 'google', 'mistral', 'cohere'));
  END IF;
  
  -- Contrainte pour temperature
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_api_configs_temperature_check' 
    AND table_name = 'user_api_configs'
  ) THEN
    ALTER TABLE user_api_configs ADD CONSTRAINT user_api_configs_temperature_check 
    CHECK (temperature >= 0 AND temperature <= 1);
  END IF;
  
  -- Contrainte pour max_tokens
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'user_api_configs_max_tokens_check' 
    AND table_name = 'user_api_configs'
  ) THEN
    ALTER TABLE user_api_configs ADD CONSTRAINT user_api_configs_max_tokens_check 
